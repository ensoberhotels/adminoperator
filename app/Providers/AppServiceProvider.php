        ...
        view()->composer('*', function($view)
        {
            ...
            if($log_user !=''){
                if($log_user['login_type'][0] == 'A'){
                    ...
                    $module_id=DB::table('sua_company_privileges')->where('company_id',$log_user['company_id'][0])->where('login_type','O')->pluck('module_id')->toArray();
                    $access_menus=[];
                    foreach($module_id as $module_id){
                        $access_menus[$module_id]=MenuMaster::where('login_type','O')->whereIn('id',$data)->where('module',$module_id)->get();
                    }
                }
