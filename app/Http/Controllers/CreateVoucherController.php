...
class CreateVoucherController extends Controller
{
    ...
    public function createvoucher()
    {
        try {
			...
			$paymentsources     =   PaymentSource::where('company_id', $session_data['company_id'][0])->get();
			return view('operator/voucher/index',compact('hotels', 'citiesh', 'booking_froms', 'booking_sources', 'paymentsources'));
     ...
     * @return \Illuminate\Http\Response
     */
    public function getVoucher($quotation_no)
    {
        try {
			$check_valid = $this->checkQuotationNoIsValid($quotation_no);
			if($check_valid == 0){
				echo "<center><h4 style='color:red;'>This Quotation No Is Invalid, Quotation No Is: <span style='color:blue'>$quotation_no</span> </h4></center><br> <center><a href='/operator/createvoucher'><button>Go Back</button></a></center>";
				exit;
			}
			$citiesh = City::select('hotels.*','cities.id as city_id','cities.name as name')->join('hotels', 'cities.id', '=', 'hotels.city_id')->groupBy('cities.id')->get();
            ...
            return view('operator/voucher/edit',compact('hotels', 'citiesh','quotation','lead','roomtypes','roomsrate','booking_detail', 'booking_froms', 'booking_sources')); 
        } catch (ModelNotFoundException $e) {
            return $e;
        }
    }
    ...
    {
		// dd($request->all());
		// dd($request->booking_from,$request->source);
		// DB::beginTransaction();
		...
		//try {
			...
			for($x = 0; $x< $count; $x++){	
				...
				if($hotel_rate['status'] == true){
					...
					$sendquotation->quotation_type 		  	= 'voucher';
