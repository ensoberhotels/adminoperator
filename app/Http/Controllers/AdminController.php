...
class AdminController extends Controller
{
	...
	public function sendMail(Request $request){
		//dd($request->all());
		$user=Admin::where('user_email',$request->user_email)->first();
		//dd($user);
		if(isset($user)){
				$token=encrypt($user->user);
				$message= '<table style="border:0px;">
					<tr>
						<td> Hi '.$user->user.'</td>
					</tr>
					<tr>
						<td><a href="/admin/reset-password/'.$token.'">Forget Password</a></td>
					</tr>
				</table>';
				$subject='Forget Password Notifications';
				$from='Sales@ensoberhotels.com';
				$to=$user->user_email;
				//dd($message,$subject,$from,$to);
				$send=$this->send($message,$subject,$from,$to);
				if($send){
					Admin::where('user_email',$request->user_email)->update(['token'=>$token]);
				}
	...
	public function adminLogin(Request $request){
		...
		try{
			$admin = Admin::where('user', $request->user)->where('password', $request->password)->where('user_type','A')->first();
			if($admin != ""){
	...
	public function viewHotel($id){
        ...
        $Hotel = Hotel::select('hotels.*', 'cities.name as city', 'countries.name as country', 'regions.name as region')
                       ...
                       ->where('hotels.id', $id)->where('company_id',$user['comp_id'][0])->where('property_id',$user['id'][0])->first();
		if ($Hotel) {
