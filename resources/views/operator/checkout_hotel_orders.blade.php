                ...
                <div class="card-content">
					...
					<div class=" active custom_tab_main tab_border">
						<div class="col s12">
							<ul class="custom_tabs">
								<li class="custom_tab">
									<a class="@if(Request::segment(2) == 'getcheckinhotelorder') active @endif;" href="{{URL::to('/operator/getcheckinhotelorder/')}}">Checkin Bookings </a>
								</li>
								<li class="custom_tab">
									<a class="@if(Request::segment(2) == 'getcheckouthotelorder') active @endif;" href="{{URL::to('/operator/getcheckouthotelorder/')}}">Checkout Bookings </a>
								</li>								
								<li class="custom_tab">
									<a class="@if(Request::segment(2) == 'getclosedhotelorder') active @endif;" href="{{URL::to('/operator/getclosedhotelorder/')}}">Closed Bookings </a>
								</li>
							</ul>
						</div>
					</div> 
								...
								<div class="col s12 m3">
									...
									<select name="hotel_id">
										...
										@foreach($hotels as $hotel)
											<option value="{{$hotel->id}}" @if(@request()->hotel_id == $hotel->id) {{ 'selected' }} @endif>{{$hotel->hotel_name}}</option>
										@endforeach
									</select>
								</div>
								<div class="col s6 m3"><br>
									<button type="submit" class="btn waves-effect gradient-45deg-red-pink waves-light" id="addbookingbtn">Search</button>
								</div>
							</div>
						</form>
					@endif
					...
					<div class="row" id="details">					
						<div class="col s12 m12">
							<h3 class="card-title follow_title"><strong>Yesterday Checkout Bookings</strong></h3>
								...
								<table class="table" style="font-size:12px;color:#000;">
									<thead>
										<tr>
											...
											<!--<th>Create Date</th>-->
											<th>Total Amount</th>
											<!--<th title="Closed From Owner">CFO</th>-->
											<!--<th title="Closed From Vendor">CFV</th>-->
									...
									<tbody>
										...
										<tr>
											...
											<td>{{$order->cost}}</td>
											<!--<td>
												{{$order->closed_f_owner}}
											</td>
											<td>
												{{$order->closed_f_vendor}}
											</td>-->
											<td style="width:150px;">
												<button type="button" class="btn waves-effect gradient-45deg-red-pink waves-light" style="margin-top: 5px !important;" onclick="showPaymentHistory('{{$order->send_quotation_no}}');">Payment</button>
												@if($order->closed_f_vendor == '1') 
													<span style="background-color: green; padding: 1px 10px; color: #fff; border-radius: 3px; margin-top: 6px !important;float: left;margin-right: 5px;">Closed</span>
												@else
													<a href="/operator/acceptorder/{{$order->send_quotation_no}}" target="_blank" style="margin-top: 4px !important;float: left;margin-right: 5px;">
														<button type="submit" class="btn waves-effect gradient-45deg-red-pink waves-light" >Accept</button>
													</a>
												@endif
											</td>
										</tr>
										<tr>
											<td colspan="16" class="{{$order->send_quotation_no}}" style="font-size:12px;border: 1px solid #eee;background-color: #2962ff0d; display:none;" border="1">
