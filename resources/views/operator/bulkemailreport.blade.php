              ...
              <table id="multi-select" class="display">
                <thead>
                  <tr>
                    ...
                    <th class="" >Date</th>                
                    <th class="" >Subject</th>                  
                    <th class="" >Email Campaign</th>                        
                    <th class="" >Email List</th> 
                    <th class="" >Send Report</th> 
                  </tr>
                </thead>
                <tbody>
				@php $i = 1  @endphp
                @foreach($sendreports as $sendreport)
                <tr>
                    <td> {{ $operator_name[0] }}</td>
                    <td> {{ date('d M Y h:i A', strtotime($sendreport->created_at)) }}</td>
                    <td> {{ $sendreport->title }}</td>
                    <td> {{ @$sendreport->EmailCampaign->title }}<br>
						<a href="{{ url('storage/app/'.@$sendreport->EmailTemplat->template) }}" target="_blank"><img src="{{ url('storage/app/'.@$sendreport->EmailTemplat->template) }}" width="100px"/></a>
					</td>
                    <td> {{ @$sendreport->EmailTemplat->title }}</td>
					<td>
					
						@if($sendreport->BulkEmailSendReport->count() > 0)
														
							<ul class="hide sendEmails" style="width: 291px; box-shadow: 0px 0 5px 0 #eee; background-color: #fff; padding: 10px 10px 21px;height: 150px; overflow-x: scroll;">
							<?php $sc = 0; ?>
							@foreach($sendreport->BulkEmailSendReport as $SendReport)
								@if($SendReport['send_status'] == 'SEND')
									<li style="float: left;width: 100%; margin: 0 0 10px;">
										{{ $SendReport['email'] }} 
										<span class="status_btn_g" style="float: right;">{{ $SendReport['send_status'] }} </span>
									</li>
									<?php $sc++; ?>
								@endif
							@endforeach
							...
							<ul class="hide unsendEmails" style="width: 291px; box-shadow: 0px 0 5px 0 #eee; background-color: #fff; padding: 10px 10px 21px;height: 150px; overflow-x: scroll;">
							<?php $unc=0; ?>
							@foreach($sendreport->BulkEmailSendReport as $SendReport)
								@if($SendReport['send_status'] == 'UNSEND')
									<li style="float: left;width: 100%; margin: 0 0 10px;">
										{{ $SendReport['email'] }} 
										<span class="status_btn_r" title="{{ $SendReport['error_message'] }}" style="float: right;"> {{ $SendReport['send_status'] }}</span>
									</li>
								<?php $unc++; ?>
								@endif
							...
							@if($sc >0)
							<button class="buttons-csv buttons-html5 mb-6 btn waves-light gradient-45deg-purple-deep-orange send_report" tabindex="0" aria-controls="multi-select" type="button" style="width: 115px; padding: 0 13px;">Send <i class="right">{{$sc}}</i></button>
							@endif
							
							@if($unc >0)
							<button class="buttons-csv buttons-html5 mb-6 btn waves-effect waves-light gradient-45deg-purple-deep-orange unsend_report " tabindex="0" aria-controls="multi-select" type="button" style="width: 115px; padding: 0 13px;">Unsend <i class="right">{{$unc}}</i></button>
							@endif
						@endif
					</td>
                </tr>
...
@section('scripts')
    ...
    $(document).ready(function() {
		...
		// Show Report
		jQuery( "table" ).delegate( ".send_report", "click", function() {
			jQuery(this).parent().find(".sendEmails").toggleClass("hide");
		});
		
		// Hide Report
		jQuery( "table" ).delegate( ".unsend_report", "click", function() {
			jQuery(this).parent().find(".unsendEmails").toggleClass("hide");
		});
