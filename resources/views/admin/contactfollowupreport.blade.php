    ...
    <div class="col s12">
      <div class="card">
        <div class="card-content">
          <div style=" border-bottom: 1px solid #ff4081; margin-bottom: 15px; margin-top: -25px;">
		<form action="" method="GET" >
      {{ csrf_field() }}
        <div class="row"> 
      <div class="input-field col s4"> 
        <label for="hotel_name" class="">From Date</label>
        <input class="validate invalid" id="fromdate" name="fromdate" type="date" value="{{ app('request')->input('fromdate') }}">
      </div>
      <div class="input-field col s4"> 
        <label for="hotel_name" class="">To Date</label>
        <input class="validate invalid" id="todate" name="todate" type="date" value="{{ app('request')->input('todate') }}">
      </div>
	  <div class="input-field col s4">
              ...
              <table id="multi-select" class="display">
                <thead>
                  <tr>
                    ...
                    <th>Operator</th>                  
                    <th>Contact FollowUp No</th>
                  </tr>
                </thead>
                <tbody>
                @foreach($Operators as $Operator)
                <tr class="animated">
                    <td class="custom_check">
						<input type="checkbox" name="Operator[]" class="custome_checkbox" value="{{$Operator->id}}"/>
                    </td>
                    <td> {{ $Operator->name }}</td>
                    <td> {{ $Operator->id }}</td>                   
                  </tr>
