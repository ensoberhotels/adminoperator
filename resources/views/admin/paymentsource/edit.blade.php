            ...
            <div class="row">
              <div class="col s10 m6 l6">
                ...
                <ol class="breadcrumbs mb-0">
                  ...
                  <li class="breadcrumb-item"><a href="#">Edit Hotel Payment Source</a>
                  </li>
            ...
            <div class="card">
                <div class="card-content">
                    <div class="card-header">
                        <h4 class="card-title">Update Hotel Payment Source</h4>
                    </div>
					
            <form class="add_hotel_group_rate_form" id="add_hotel_group_rate_form" method="POST" action="{{ route('paymentsource.update',$paymentsource->id) }}" >
            {{csrf_field()}}
                            ...
                            <div class=" col s12 m2">
                                ...
                                <option value="">Please Select Hotel </option>
                                    ...
                                    @foreach($hotels as $hotel)
                                        <option value="{{ $hotel->id }}" @if($hotel->id==$paymentsource->hotel_id) selected @endif >{{ $hotel->hotel_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class=" col s12 m2">
                                <label for="source" class="">Payment Source</label>
                                <input class="validate invalid" required="" aria-required="true" id="source" type="text" name="source" value="{{ $paymentsource->source }}">
                            </div>
							<div class=" col s12 m2">
                                <label for="owner" class="">Owner Payment Method</label>
								<select name="owner" id="owner">
									<option value="0" @if($paymentsource->owner=='0') selected @endif>No</option>
									<option value="1" @if($paymentsource->owner=='1') selected @endif>Yes</option>
								</select>
                            </div>
                            <div class="col s12 m2">
							  <p>Status</p>
							  <p>
								<label>
								  <input class="validate" required="" aria-required="true" value="ACTIVE" name="status" type="radio" @if($paymentsource->status=='ACTIVE') checked @endif>
								  <span>ACTIVE</span>
								</label>
							  </p>

							  <label>
								<input class="validate" required="" aria-required="true" value="INACTIVE" name="status" type="radio" @if($paymentsource->status=='INACTIVE') checked @endif>
								<span>INACTIVE</span>
							  </label>
							  <div class="">
							  </div>
