						...
						<div class="col s6" style="padding-left: 25px;">
							<table>
								...
								<tr>
									...
									<td><i class="material-icons">chevron_right</i></td>
									<td> <input type="datetime-local" name="drop_date_time" id="drop_date_time" class="filter_date1" autocomplete="off" placeholder="Drop Date"  min=''/></td>
								</tr>
								<tr>
									<td>Drop City</td>
									<td><i class="material-icons">chevron_right</i></td>
							...
							<table id="add_more_dis">
								<tr>
									...
									<td>
										...
										<button class="btn waves-effect waves-light" type="button" onclick="oprnRoutePop(0)"><i class="material-icons">alt_route</i> 
										</button>
			...
			jQuery("#save_basic_info").click(function(){
				...
				if(adult != "" && tour_type != "" && arrival_date_time != "" && arrival_city != "" && drop_date_time != "" && drop_city != ""){
				  ...
				  $.ajax({
					...
					cache: false,
					async:false,
					data: {'adult':adult,'kids':kids,'infant':infant,'tour_type':tour_type, 'arrival_date_time':arrival_date_time, 'arrival_city':arrival_city, 'drop_date_time':drop_date_time, 'rate_show':rate_show, 'drop_city':drop_city, 'itinerary_no_val':itinerary_no_val,'name':name, 'mobile':mobile, 'comment':comment},
					success: function(data){
						var data = JSON.parse(data);
						console.log(data);
